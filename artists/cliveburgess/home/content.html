...
const clickableElements = document.querySelectorAll('a, .baby-image-container, .click-zone');
...
document.addEventListener('click', (e) => {
    ...
    const babyImage = document.querySelector('.baby-image-container');

    // Check if click is NOT on the click zone or baby image
    if (!clickZone.contains(e.target) && !babyImage.contains(e.target)) {
